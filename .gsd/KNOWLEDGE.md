...
| 20 | ContextVar per-request accumulation for deep-to-middleware data flow | `backend/app/middleware/timing.py`, `backend/app/triplestore/client.py` | After M049/S03, per-request SPARQL timing data flows from TriplestoreClient (deep service layer) to TimingMiddleware (response header serialization) via a `_sparql_timings` ContextVar. The middleware initializes the ContextVar before `call_next()`, deep service code calls `record_sparql_timing()` which appends to it, middleware serializes entries into the Server-Timing header in a `finally` block. The helper is a no-op when the ContextVar is not set (background tasks, tests). Use this pattern whenever request-scoped data needs to flow upward from service layers to middleware without threading context through every function signature. |
| 21 | OTel unit tests require _force_set_tracer_provider() to bypass global Once guard | `backend/tests/test_tracing.py` | OTel's `trace._TRACER_PROVIDER` is protected by a Once guard that prevents re-initialization. In unit tests, each test needs a fresh TracerProvider with its own InMemorySpanExporter. The workaround: `trace._TRACER_PROVIDER = None`, then `trace._TRACER_PROVIDER_SET_ONCE = Once()`, then re-bind module-level tracers via the module's `_tracer = trace.get_tracer(...)`. Also need `trace._TRACER_PROVIDER_FACTORY = lambda: trace.NoOpTracerProvider()` for proper reset. Any future OTel test file should import this helper from test_tracing.py or replicate the pattern. |
| 22 | Tar archive downloads must use `validate_tar_contents()` + `safe_extract()` from `backend/app/security/tar_validator.py` before extraction | `backend/app/services/marketplace.py` | After M053/S02, the tar validator mirrors the zip_validator.py API shape. Six checks: path traversal, absolute paths, symlinks, hardlinks, uncompressed size (default 2GB), file count (default 50k). Archive-level compression ratio heuristic (tar lacks per-entry compressed sizes — uses total archive size / member count). `safe_extract()` validates then extracts using Python 3.12's `tarfile.data_filter` for defense-in-depth. New endpoints accepting tar.gz uploads must call `validate_tar_contents(tar_obj)` and catch `ValueError`. |
| 23 | Safe update ordering for downloadable assets: download → verify → extract → confirm → THEN remove old → install new | `backend/app/services/marketplace.py`, `backend/app/admin/router.py` | After M053/S03, the model update endpoint downloads and fully verifies the new archive before removing the old installation. If download or verification fails, the old model remains intact. Never remove-then-download — a network failure mid-download leaves the user with neither version. Apply this pattern to any future asset update flow (themes, plugins, extensions). |
| 24 | Explorer config `prop:` prefix convention for SHACL-derived property dropdowns | `backend/app/browser/explorer_config.py`, `frontend/static/js/explorer-config.js` | After M054/S01, config-options API returns type-specific SHACL properties prefixed with `prop:` (e.g. `prop:http://example.org/status`) to distinguish them from built-in options (`type`, `tag`, `label`, `created`). `ExplorerConfig.__post_init__` strips the `prop:` prefix before SPARQL interpolation. Frontend sends `prop:`-prefixed values; backend strips. Any new config option category (e.g. filter-by-property) should follow this convention — prefix identifies origin, backend strips at the dataclass boundary. |
| 25 | Multi-instance UI components use Map keyed by DOM element for per-section state | `frontend/static/js/explorer-config.js` | After M054/S02, duplicated OBJECTS sections each get independent state stored in a `Map` keyed by the section DOM element. All DOM access uses class-based selectors scoped to `sectionRoot.querySelector()` instead of `document.getElementById()`. When cloning UI sections that need independent state, use this pattern — IDs must be unique per document, so ID-based access breaks on duplication. |
