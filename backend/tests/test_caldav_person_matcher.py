"""Unit tests for the CalDAV person matcher.
...
_person_matcher = _load_module(
    "caldav_person_matcher", _SERVICES_DIR / "person_matcher.py"
)
...
class TestPersonMatcherEmailLookup:
    ...
    @pytest.mark.asyncio
    async def test_sparql_query_checks_both_email_predicates(self):
        """Query should check foaf:mbox and crm:email via UNION."""
        graph = MockGraphClient()
        commands = MockCommandClient()
        matcher = PersonMatcher(graph, commands)

        await matcher.match_or_create("test@example.com", "Test")

        assert len(graph.queries) == 1
        sparql = graph.queries[0]
        assert "foaf" in sparql.lower()
        assert "crm:email" in sparql.lower() or "crm" in sparql.lower()
...
class TestPersonMatcherCache:
    ...
    @pytest.mark.asyncio
    async def test_cache_stores_created_person(self):
        """After creating a person, the IRI should be cached for subsequent calls."""
        graph = MockGraphClient()
        commands = MockCommandClient()
        matcher = PersonMatcher(graph, commands)

        result1 = await matcher.match_or_create("new@example.com", "New Person")
        result2 = await matcher.match_or_create("new@example.com", "New Person")

        assert result1 == result2
        assert len(graph.queries) == 1  # only one SPARQL query
        assert len(commands.commands) == 1  # only one create command
...
class TestPersonMatcherEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_person_type_is_bpkm(self):
        """Created person should use the bpkm:Person type IRI."""
        graph = MockGraphClient()
        commands = MockCommandClient()
        matcher = PersonMatcher(graph, commands)

        await matcher.match_or_create("test@test.com", "Test User")

        cmd = commands.commands[0]
        assert cmd["params"]["type"] == "urn:sempkm:model:basic-pkm:Person"
...
class TestSlugify:
    ...
    def test_unicode_removed(self):
        assert _slugify("café résumé") == "caf-rsum"


class TestEmailLocalPart:
    """Test the internal _email_local_part function."""

    def test_standard_email(self):
        assert _email_local_part("user@example.com") == "user"

    def test_no_at_sign(self):
        assert _email_local_part("noemail") == "noemail"
