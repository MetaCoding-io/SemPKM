<!-- GENERATED FILE. Do not edit. Source: models/research/README.md. Regenerate with: python3 scripts/sync-model-docs.py -->
> **Bundled model documentation.** This page mirrors the `README.md` shipped inside the `research` Mental Model archive. The same text is available in the app under **Admin > Models > Research Workflow > Documentation** and at `/api/models/research/docs`. To change it, edit `models/research/README.md` and run `scripts/sync-model-docs.py`.
