...
> **For Advanced Users:** The console supports SELECT queries only. CONSTRUCT and DESCRIBE queries can be run via the SPARQL API endpoint (`/api/sparql`) directly -- see [Chapter 20: The SPARQL Endpoint](20-sparql-endpoint.md). UPDATE and DELETE operations are not supported through the console or API; all data modifications must go through the command API to ensure proper event logging, validation, and webhook dispatch. If you need to query the Lucene full-text index from SPARQL, see the `luc:matches` predicate described in [Chapter 25: Keyword Search](25-keyword-search.md).
...
**Previous:** [Chapter 23: Hosted Demo Instance](23-hosted-demo.md) | **Next:** [Chapter 25: Keyword Search](25-keyword-search.md)
