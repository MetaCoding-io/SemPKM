...
> **For Advanced Users:** The console supports SELECT queries only. CONSTRUCT and DESCRIBE queries can be run via the SPARQL API endpoint (`/api/sparql`) directly -- see [Chapter 19: The SPARQL Endpoint](19-sparql-endpoint.md). UPDATE and DELETE operations are not supported through the console or API; all data modifications must go through the command API to ensure proper event logging, validation, and webhook dispatch. If you need to query the Lucene full-text index from SPARQL, see the `luc:matches` predicate described in [Chapter 24: Keyword Search](24-keyword-search.md).
...
**Previous:** [Chapter 22: Hosted Demo Instance](22-hosted-demo.md) | **Next:** [Chapter 24: Keyword Search](24-keyword-search.md)
