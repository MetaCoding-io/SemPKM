...
plan: "02"
subsystem: backend-api, nginx
tags: [llm, sse, streaming, httpx, nginx, proxy]
dependency-graph:
  requires: [17-01]
  provides: [llm-chat-stream-endpoint, nginx-sse-location]
  affects: [backend/app/browser/router.py, frontend/nginx.conf]
tech-stack:
  added: []
  patterns: [SSE streaming via httpx.AsyncClient.stream, aiter_lines for line-buffered SSE, StreamingResponse text/event-stream, nginx proxy_buffering off]
key-files:
  created: []
  modified:
    ...
    - frontend/nginx.conf
decisions:
  - aiter_lines() used over aiter_bytes() to ensure complete SSE lines across HTTP chunk boundaries
  - 300s timeout for httpx AsyncClient to handle long LLM completions
  - X-Accel-Buffering no header on StreamingResponse as defense-in-depth even without nginx location match
  - Endpoint accessible to any authenticated user (get_current_user not require_role) for future copilot use
  - nginx SSE location block placed explicitly before catch-all location / as conventional best practice
metrics:
  duration: 1min
