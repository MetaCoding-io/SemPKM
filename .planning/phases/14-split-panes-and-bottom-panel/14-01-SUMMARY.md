...
plan: "01"
subsystem: workspace
tags: [editor-groups, split-panes, workspace-layout, split-js, session-storage]
dependency_graph:
  requires: []
  provides:
    - WorkspaceLayout class with addGroup/removeGroup/moveTab/addTabToGroup/removeTabFromGroup
    - initWorkspaceLayout() entry point for multi-group DOM initialization
    - getActiveEditorArea() replacing all hard-coded #editor-area references
    - splitRight() / setActiveGroup() / switchTabInGroup() / closeTabInGroup() window APIs
    - sempkm_workspace_layout sessionStorage schema (migrated from sempkm_open_tabs)
    - editor-column / editor-groups-container DOM structure in workspace.html
    - VS Code-style gutter CSS (.gutter-editor-groups) with hover hit-target expansion
  affects:
    - frontend/static/js/workspace.js (tab delegation, keyboard shortcuts)
    - backend/app/templates/base.html (script load order)
tech_stack:
  added: []
  patterns:
    - Split.js destroy-and-recreate for dynamic editor groups
    - IIFE module pattern (consistent with existing codebase)
    - sessionStorage migration: old keys -> new sempkm_workspace_layout schema
    - getActiveEditorArea() helper pattern for multi-group htmx targeting
key_files:
  created:
...
decisions:
  - Ctrl+\\ reassigned from toggleSidebar to splitRight (sidebar stays on Ctrl+B per Phase 12)
  - workspace-layout.js as separate IIFE module, not inline in workspace.js
  - gutterSize:1 with CSS ::after pseudo-element for wider hit target (Pitfall 2 solution)
  - splitRight duplicates active tab into new group (Duplicate objects allowed per CONTEXT.md)
  - Tab normalization: ensure both .id and .iri fields are set on every tab object
metrics:
  duration: "4min"
