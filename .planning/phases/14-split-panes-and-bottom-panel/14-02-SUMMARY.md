...
plan: "02"
subsystem: workspace
tags: [drag-and-drop, context-menu, tab-management, editor-groups, html5-dnd]
dependency_graph:
  requires:
    - 14-01 (WorkspaceLayout class, renderGroupTabBar, moveTab, editor-groups DOM)
  provides:
    - initTabDrag(tabEl, tabId, groupId) HTML5 drag initiation with ghost image
    - initTabBarDropZone(tabBarEl, targetGroupId) drop zone with insertion indicator
    - _updateInsertionIndicator() visual drop position line
    - initRightEdgeDropZone() right-edge new-group creation (last 80px)
    - showTabContextMenu(e, tabId, groupId) right-click context menu with Close/Close Others/Split Right/Move to Group
    - closeOtherTabsInGroup(keepTabId, groupId) helper
    - isDragging flag with Pitfall 3 guard (setTimeout reset)
    - Phase 14 DnD and context menu CSS in workspace.css
  affects:
...
tech_stack:
  ...
  patterns:
    ...
    - dataTransfer.setData/getData for passing tab identity across groups
    - setDragImage with off-screen ghost clone (position fixed, top -1000px)
    - isDragging flag with setTimeout(0) reset to guard against accidental tab switch after short drag
    - getBoundingClientRect mid-point calculation for insertion indicator positioning
    - Pattern 6 from 14-RESEARCH.md for context menu: viewport clamped, setTimeout dismiss guard, Escape keydown
key_files:
  created: []
  modified:
    - frontend/static/js/workspace-layout.js
    - frontend/static/css/workspace.css
decisions:
  - isDragging reset uses setTimeout(0) on dragend, matching Pitfall 3 solution from RESEARCH.md
  - renderGroupTabBar refactored from innerHTML string-building to DOM element construction to support addEventListener attachment
  - recreateGroupSplit now explicitly creates editor-groups-container div and preserves/recreates bottom-panel-slot
  - initTabBarDropZone called even when tab bar is empty (drop zone active for empty groups)
  - Context menu Move to Group uses layout.groups.indexOf(g)+1 for display number (position-stable)
  - color-mix() used in drag-over CSS (already present in project; shadow-elevated fallback added for context menu box-shadow)
metrics:
  duration: "3min"
