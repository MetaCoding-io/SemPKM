...
plan: 02
subsystem: ui
tags: [htmx, jinja2, css, event-log, filters, pagination]

# Dependency graph
requires:
  - phase: 16-event-log-explorer
    plan: 01
    provides: "GET /browser/events route with active_filters/current_params context, event_log.html stub header, EventQueryService"
provides:
...
tech-stack:
  added: []
  patterns:
    - "dict_without|urlencode Jinja2 filter chain: remove param from current_params dict, encode to query string for chip remove URL"
    - "Filter chip remove pattern: hx-get with all current params minus the removed one, hx-target=#panel-event-log, hx-swap=innerHTML"
    - "hx-include multi-input pattern: each filter input includes peer inputs by ID for AND-logic query building"
    - "color-mix(in srgb, color 15%, transparent) for badge tinted backgrounds (consistent with existing workspace.css usage)"
